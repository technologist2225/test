    ...
    try:
        ...
        sql_product = """
            SELECT ProductID AS "ProductID", ProductName AS "ProductName", Price AS "Price", 
                   StockQuantity AS "StockQuantity", CategoryID AS "CategoryID", ShopID AS "ShopID", 
        ...
        if not product:
            cursor.close(); conn.close()
            return False, "Không tìm thấy sản phẩm hoặc sản phẩm đã ngừng kinh doanh", None
            
        sql_images = """
            SELECT ImageID AS "ImageID", ImageURL AS "ImageURL", IsPrimary AS "IsPrimary"
            FROM ProductImages WHERE ProductID = %s ORDER BY IsPrimary DESC, ImageID ASC; 
        """
...
def create_product(product_name, price, stock_quantity, category_id, shop_id, user_id, role_id, image_urls=None):
    ...
    try:
        ...
        # BẢO MẬT: Kiểm tra quyền sở hữu cửa hàng
        if role_id != 1: # Nếu không phải Admin thì phải check
...
def update_product(product_id, user_id, role_id, product_name=None, price=None, stock_quantity=None, category_id=None, image_urls=None):
    ...
    try:
        ...
        # BẢO MẬT: Kiểm tra quyền sở hữu sản phẩm (Join bảng Shop để lấy ManagerID)
        if role_id != 1:
            cursor.execute("""
...
def get_product_details(product_id):
    """Lấy chi tiết SP (Cập nhật lấy thêm Mô tả, Lượt bán, và Thông số kỹ thuật)"""
    conn = get_db_connection()
    if not conn: return False, "Lỗi kết nối Cơ sở dữ liệu", None
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        # THÊM CỘT Description VÀ SoldQuantity
        sql_product = """
            SELECT ProductID AS "ProductID", ProductName AS "ProductName", Price AS "Price",
        ...
        # LẤY THÔNG SỐ KỸ THUẬT (BẢNG MỚI)
...
def get_product_rating_stats(product_id):
    """Nghiệp vụ lấy Thống kê sao VÀ Danh sách chi tiết các bài đánh giá"""
    conn = get_db_connection()
    if not conn: return False, "Lỗi kết nối", None
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        
        # 1. Lấy thống kê tổng quan (Trung bình sao và Tổng số lượt)
        cursor.execute("""
        ...
        # 2. Lấy danh sách chi tiết các đánh giá (JOIN với Users để lấy Tên và Avatar)
        cursor.execute("""
            SELECT 
