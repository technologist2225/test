...
def get_all_orders(page=1, limit=10, user_id=None, role_id=None):
    """Hàm lấy danh sách đơn hàng có phân quyền an toàn tuyệt đối"""
    conn = get_db_connection()
    if not conn: return False, "Lỗi kết nối", None
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        
        # BẢO MẬT: Lọc dữ liệu theo Role
        where_clause = ""
        params = []
        if role_id == 3: # Khách hàng chỉ thấy đơn của mình
            where_clause = "WHERE UserID = %s"
            params.append(user_id)
        elif role_id == 2: # Shop chỉ thấy đơn của shop mình
        ...
        sql_query = f"""
            SELECT 
                ...
                PaymentStatus AS "PaymentStatus", ShippingAddress AS "ShippingAddress"
            FROM Orders
            {where_clause}
...
def get_order_details(order_id, user_id, role_id):
    ...
    try:
        ...
        # BẢO MẬT: Kiểm tra quyền xem chi tiết đơn
...
def create_order(user_id, shop_id, shipping_address, items_list):
    conn = get_db_connection()
    if not conn: return False, "Lỗi kết nối", None
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        ...
        sql_insert_order = """
            INSERT INTO Orders (UserID, ShopID, TotalAmount, ShippingAddress, Status, PaymentStatus)
            VALUES (%s, %s, %s, %s, 'Chờ xác nhận', 'Chưa thanh toán')
            RETURNING OrderID::text AS "OrderID", OrderDate AS "OrderDate", Status AS "Status", PaymentStatus AS "PaymentStatus";
        """
        cursor.execute(sql_insert_order, (user_id, shop_id, total_amount, shipping_address))
        ...
        for v_item in valid_items:
            ...
            """, (v_item['Quantity'], v_item['ProductID']))
...
def update_order_status(order_id, new_status, user_id, role_id):
    ...
    try:
        ...
        # BẢO MẬT: Khách hàng (Role 3) KHÔNG có quyền tự cập nhật trạng thái đơn (chỉ có thể Hủy)
        if role_id == 3:
            cursor.close(); conn.close()
        ...
        # BẢO MẬT: Manager chỉ cập nhật được đơn của shop mình
        if role_id == 2:
...
def cancel_order(order_id, user_id, role_id):
    ...
    try:
        ...
        # BẢO MẬT: Xác thực quyền hủy đơn
        ...
        conn.commit()
        cursor.close(); conn.close()
        return True, "Đã hủy đơn hàng và hoàn tất xử lý", dict(canceled_order)
    except Exception as e:
        if conn: conn.rollback()
        return False, str(e), None
...
def get_order_payment_status(order_id, user_id, role_id):
    ...
    try:
        ...
        # BẢO MẬT: Kiểm tra quyền xem trạng thái tiền
        if role_id == 3 and str(order['userid']) != str(user_id):
            cursor.close(); conn.close()
            return False, "Bạn không có quyền xem đơn hàng này", None
        elif role_id == 2:
        ...
        del order_dict['userid'] # Xóa đi cho gọn json trả về
...
def confirm_mock_payment(order_id, user_id, role_id):
    ...
    try:
        ...
        if not order:
            conn.rollback(); cursor.close(); conn.close()
            return False, "Không tìm thấy đơn hàng để thanh toán", None

        # BẢO MẬT: Cực kỳ quan trọng -> Chỉ Khách hàng tạo đơn đó mới được tự móc ví ra trả tiền
        if role_id == 3 and str(order['userid']) != str(user_id):
            conn.rollback(); cursor.close(); conn.close()
            return False, "Bạn không có quyền thanh toán đơn hàng của người khác", None
        elif role_id == 2:
            conn.rollback(); cursor.close(); conn.close()
            return False, "Shop không thể tự thanh toán thay cho khách hàng", None
        ...
        cursor.execute("""
            UPDATE Orders SET PaymentStatus = 'Đã thanh toán' WHERE OrderID = %s
        ...
        return True, "Xác nhận thanh toán đơn hàng thành công!", result_dict
    except Exception as e:
        if conn: conn.rollback()
        return False, str(e), None