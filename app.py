...
def api_create_shop():
    try:
        ...
        # BẢO MẬT: Lấy ID của người đang đăng nhập làm Quản lý cửa hàng
        manager_id = get_jwt_identity()

        if not shop_name:
            return error_response("Vui lòng cung cấp Tên cửa hàng (ShopName)", 400)

        is_success, message, result_data = create_shop(shop_name, address, hotline, manager_id)
...
def api_update_shop(shop_id):
    try:
        ...
        # Truyền thông tin xuống Service để kiểm tra quyền
        is_success, message, result_data = update_shop(shop_id, user_id, role_id, shop_name, address, hotline)
...
def api_toggle_shop_status(shop_id):
    try:
        # BẢO MẬT: Chỉ Admin mới có quyền khóa/mở cửa hàng
        claims = get_jwt()
...
def api_submit_product_review(product_id):
    try:
        ...
        user_id = get_jwt_identity() # Cực chuẩn, lấy trực tiếp từ Token
        
        try:
            rating = int(rating_raw)
            if rating < 1 or rating > 5: return error_response("Điểm đánh giá phải từ 1 đến 5 sao", 400)
        except (ValueError, TypeError):
            return error_response("Định dạng số sao không hợp lệ", 400)

        is_success, message, result_data = submit_product_review(product_id, user_id, rating)
        if is_success: return jsonify(success_response(message, result_data)), 201 
        return error_response(message, 400)
    except Exception as e: return server_error_response(e)
...
def api_create_order():
    try:
        ...
        if not shop_id_raw or not shipping_address or not items_raw:
            return error_response("Vui lòng điền đầy đủ UserID, ShopID, Địa chỉ giao hàng và Danh sách món hàng", 400)
        ...
        try:
            ...
            return error_response("Định dạng món hàng không hợp lệ", 400)

        is_success, message, result_data = create_order(user_id, shop_id, shipping_address, items_list)
        if is_success: return jsonify(success_response(message, result_data)), 201
        return error_response(message, 400)
    except Exception as e: return server_error_response(e)
