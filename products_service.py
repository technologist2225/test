...
def submit_product_review(product_id, user_id, rating):
    conn = get_db_connection()
    if not conn: return False, "Lỗi kết nối", None
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        sql_query = """
            INSERT INTO ProductReviews (ProductID, UserID, Rating)
            VALUES (%s, %s, %s)
            RETURNING ReviewID, ProductID, UserID, Rating, CreatedAt;
        """
        cursor.execute(sql_query, (product_id, user_id, rating))
        ...
        if conn: conn.rollback()
        error_msg = str(e)
        if "users" in error_msg.lower(): return False, "Người dùng không tồn tại", None
        return False, "Sản phẩm không tồn tại", None
    except Exception as e:
        if conn: conn.rollback()
        return False, f"Lỗi cơ sở dữ liệu: {str(e)}", None
...
def get_product_rating_stats(product_id):
    ...
    try:
        ...
        return True, "Lấy thống kê đánh giá thành công", result
    except Exception as e:
        if conn: conn.rollback()
        return False, f"Lỗi cơ sở dữ liệu: {str(e)}", None